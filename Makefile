...
LDFLAGS_DEBUG= -g
EMCCFLAGS = --no-entry --preload-file inc/mandelbrot.atl@inc/mandelbrot.atl --preload-file inc/fire.atl@inc/fire.atl -s EXPORTED_RUNTIME_METHODS=["ccall"] -O3 -s WASM=1 -s USE_WEBGL2=1 -s USE_SDL=2 -s MAX_WEBGL_VERSION=3 -s MIN_WEBGL_VERSION=2 --shell-file shell.html
TARGET = Atlas.exe
HTML = Atlas.html
JS = $(HTML:.html=.js)
WASM = $(HTML:.html=.wasm)
