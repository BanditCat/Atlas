  ...
  if (dimIndex == shape_length - 1) {
    ...
    *ptr = '\0';
    ...
    if (isHorizontal) {
      ...
      u32 maxHeight = 0;
      char*** blockLinesArray = (char***)mem(sizeof(char**) * num_blocks, u8);
      ...
      for (u32 i = 0; i < num_blocks; i++) {
        ...
        char** blockLines = (char**)mem(sizeof(char*) * (lineCount + 2), u8);
        blockLines[0] = top;
      ...
      for (u32 i = 0; i < num_blocks; i++) {
        ...
        if (currentHeight < maxHeight) {
          ...
          blockLinesArray[i] = (char**)realloc(
              blockLinesArray[i],
      ...
      for (u32 i = 0; i < num_blocks; i++) {
        ...
        for (u32 j = 0; j < blockLineCounts[i]; j++) {
          ...
          if (currLen < actualMaxWidth) {
            ...
            blockLinesArray[i][j] = (char*)realloc(
              blockLinesArray[i][j], actualMaxWidth + 1
            );
...
char* formatTensorData(tensor* t) {
  ...
  char* result = helper(0, 0, 0, shape, strides, shape_length,
                        t->data, maxNumLength, t);
